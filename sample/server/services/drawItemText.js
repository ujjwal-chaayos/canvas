...
async function writePrice(screen, prices, item_id, id, itemCoordinates,itemStyle) {

  let style =
        itemStyle.weight.Prices +
        " " +
        itemStyle.size.Prices +
        " " +
        itemStyle.font.Prices;
  ...
  for (let j = 0; j < priceArray.length; j++) {
    if (priceArray[j].item_id === item_id) {
      ...
      if (priceList.length === 1) {
        ...
        await drawText(screen, priceText, pricePoints, style);
      }
      if (priceList.length > 1) {
    ...
    if (itemCoordinates[i].type === "Items") {
      ...
      for (let k = 0; k < itemArray.length; k++) {
        ...
        if (itemArray[k].active === false) {
          ...
          rectpoint.h = 56 + 10;
          await roundedRect(screen, rectpoint, 20, "grey");
        ...
        if (itemArray[k].new === true && itemArray[k].active) {
          ...
          rectpoint.w = Math.ceil(itemCoordinates[i].w * (10 / 8)) + 10;
          rectpoint.h = 56 + 10;
          await newItemRect(screen, rectpoint, 30, "yellow", "orange");
        ...
        await drawText(screen, text, points, style);

        await writePrice(screen ,prices, item_id, id, itemCoordinates,itemStyle);
...
async function doMyTitlePrint(titleCoordinate, titles, titleStyle, screen) {
  ...
  for (let i = 0; i < titleCoordinate.length; i++) {
    if (titleCoordinate[i].type === "Heading") {
      ...
      await drawText(screen, titleText, points, style);

      points.y = y + 10;
      await drawLine(screen, points, destPoint, style);
    }
  }
}
...
async function doMyWork(imageBuffer, jsondata, coordinateJson, bufferLength) {
  ...
  fs.writeFileSync(
    "./data/images" + bufferLength + ".png",
    screen1canvas.toBuffer("image/png")
  );
  ...
  await screen.save();
  fs.writeFileSync(
    "./data/screen1Menu" + bufferLength + ".png",
    screen1canvas.toBuffer("image/png")
  );
