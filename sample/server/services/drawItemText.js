...
async function drawRandF(priceX, priceY, priceW, itemStyle, screen, screen2) {
  ...
  let style = "900" + " " + size.toString() + "px" + " " + itemStyle.font.Items;
  screen.font = style;
  screen2.font = style;
  ...
  for (let i = 0; i < itemCoordinates.length; i++) {
    if (itemCoordinates[i].type === "Items") {
      ...
      if (
        ...
        let itemSecond = itemArray.slice(
          numberofItemWrap + 1,
          itemArray.length
        );
...
async function doMyWork(imageBuffer, jsondata, coordinateJson, bufferLength) {
  ...
  console.log("image created for screen1");
  ...
  console.log("image created for screen2");
  ...
  console.log("doMyWork finished!!!");
  return {
    1: buffer1,
    2: buffer2,
    screen1: screen,
    screen2: screen2,
    data1: screen1canvas.toBuffer("image/png"),
    data2: screen2canvas.toBuffer("image/png"),
  };
}
...
const drawItemText = async (imageArray, mapping, coordinates,  cafeIds, videoFlag) => {
  ...
  //console.log(screenId,templateId,background,template);
  ...
  try {
    ...
    console.log("Menu data has been read from file...");
    menuJson = data;
  } catch (err) {
    console.error("err");
  }
  ...
  //   .on("error", (e) => console.log(e))
  //   .run();
  console.log("returning response of drawItemText");
  return response[0];
};
